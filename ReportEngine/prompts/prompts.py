...
7. **大纲保真度（最高指令）**：绝对禁止在 tocPlan 中生成原始模板 `templateOverview` 以外的任何新标题、新层级或新章节！即使你认为有重要的信息需要单独成节，你也必须把它整合到现有的目录结构中。
...
5. **绝对禁止新增任何模板中没有的章节**，只能为传入的 `templateOverview` 中的现有章节分配字数；
